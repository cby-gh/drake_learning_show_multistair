...
const drake::VectorX<T> state,drake::VectorX<T> *pt_target_check,int knot_index)
{
            ...
            const Eigen::Vector3d pt_float_contact(0.13, 0, -0.30);//以foot作为落足点30//这里需要重新设置,保证精确0.10, 0, -0.29
...
void runDircon(
    ...
    vector<PiecewisePolynomial<double>> init_vc_traj) 
 {
    ...
    Vector3d pt(0.13, 0, -0.30);//以foot作为落足点30//这里需要重新设置,保证精确0., 0, -0.30 
...
//                                                             left_lower_leg, Matrix3d::Identity(), Vector3d::Zero(), {0, 2});//0 1 2对应所有xyz方向， 激活方向0 1 2 = x y z
 ...
 //double min_T = .1;//The minimum total duration of the mode (default 0)
  ...
  double  min_T_mode1 = 0.25;//在这里直接修改左右相时间相等或者在类里添加时间相等约束
  double max_T_mode1 = 0.25;//
...
//Adds a duration constraint on the total duration of the trajectory. 
 trajopt.AddDurationBounds(duration/2, duration/2);//duration = 1.0s for 2 mode duration of the trajectory. 
...
//set solver
  ...
  double tol =1e-3;
  trajopt.SetSolverOption(drake::solvers::SnoptSolver::id(),"Major feasibility tolerance", tol);  // target complementarity gap
  trajopt.SetSolverOption(drake::solvers::SnoptSolver::id(), "Major optimality tolerance", tol);  // target nonlinear constraint violation
   trajopt.SetSolverOption(drake::solvers::SnoptSolver::id(),"Minor feasibility tolerance", tol);  // target complementarity gap
...
// planar_z = 1
  ...
  trajopt.AddLinearConstraint(xf(positions_map["base_x"]) == 1.35*stride_length);//用buonding constriants4*
  ...
  trajopt.AddLinearConstraint(xf(positions_map["joint_left_rollmotor_yawmotor"]) >=-0.08 );
  ...
  trajopt.AddLinearConstraint(x0(positions_map["joint_left_pitchmotor_thigh"]) ==xf(positions_map["joint_right_pitchmotor_thigh"]) );
  trajopt.AddLinearConstraint(x0(positions_map["joint_right_pitchmotor_thigh"]) ==xf(positions_map["joint_left_pitchmotor_thigh"]));
  trajopt.AddLinearConstraint(x0(positions_map["joint_right_knee"]) ==xf(positions_map["joint_left_knee"]));
  trajopt.AddLinearConstraint(x0(positions_map["joint_left_knee"]) ==xf(positions_map["joint_right_knee"]));
...
//   }
  ...
  const double w_q_quat=1.0;
  const double w_lambda_diff=1.0;
  const double w_u_diff=1.0; 
  const double w_v_diff=1.0; 
  const double w_q_quatdot=1.0;
  double w_q_hip_rolldot = 4.0;
  double w_q_hip_yawdot = 4.0;
  const double R = 1;  // Cost on input 
  const double Q = 1;  // Cost on total velocity
    const double O = 1;  // Cost on total position 

  const double L = 10;  // x_left_hip
  const double M = 8;  // x_right_hip
  const double N = 3;  // x_left_knee
...
//std::cout << "222" << std::endl;//finding_path2 <<std::endl;//drake/examples/PlanarWalker/PlanarWalker2.urdf
  trajopt.CreateVisualizationCallback(
      drake::FindResourceOrThrow("drake/examples/v10/threeD_v2_flaypart_show.urdf"),//e
  ...
  std::cout <<"result.is_success():"<<result.is_success()<<std::endl;
   ...
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(721)<<std::endl;//lammda0
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(751)<<std::endl;//lammdac0
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(778)<<std::endl;//gammac0
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(956)<<std::endl;//lammda1
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(986)<<std::endl;//lammdac1
   std::cout<<"trajopt.decision_variable(i): "<<trajopt.decision_variable(1013)<<std::endl;//gammac1
  ...
  std::cout<<"decision_variables_value.size()"<<decision_variables_value.size()<<std::endl;
    ...
    out_lammda_data_file0.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/lammda_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::trunc);//输出且写入清空
    out_lammda_c_data_file0.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/lammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::trunc);
    out_gamma_c_data_file0.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/gammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::trunc);
  ...
  gamma_c_index0=gamma_c_index0-3;
    ...
    out_lammda_data_file1.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/lammda_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::app);
    out_lammda_c_data_file1.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/lammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::app);
    out_gamma_c_data_file1.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/gammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::app);
    out_lammda_data_file1 << std::fixed;
    out_lammda_c_data_file1 << std::fixed;//不用科学计数法
    out_gamma_c_data_file1 << std::fixed;//不用科学计数法
    int lammda_index1 = 956;
    for(int i=0;i<10;i++)
    {
      for(int j=0;j<3;j++)
      {
  ...
  //   out_lammda_data_file2.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/lammda_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::app);
  //   out_lammda_c_data_file2.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/lammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::app);
  //   out_gamma_c_data_file2.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/gammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::app);
...
for(int i = 0; i<builder.GetMutableSystems().size();i++)
{
 ...
 auto diagram = builder.Build();
   ...
   double trajectory_time_x =  pp_xtraj.end_time()-pp_xtraj.start_time();
    ...
    out_u_data_file.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/u_planed_outsearch.txt", std::ios::out | std::ios::trunc);
    out_x_data_file.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/x_planed_outsearch.txt", std::ios::out | std::ios::trunc);
                                  ...
                                  VectorXd* prior_q_last_guess = NULL) 
  {
    ...
    Eigen::Vector4d quat(2000.06, -0.339462, -0.609533, -0.760854);
        ...
        for(int i=((mode_index>=1)?1:0);i<mode_length;i++)//逐点的求取逆运动学初始轨迹
        {
          ...
          {
          ik.AddPositionConstraint(toe_left_frame, Vector3d(0.13, 0, -0.30), world_frame, stance_toe_pos - eps_vec, stance_toe_pos + eps_vec);
          ik.AddPositionConstraint(toe_right_frame, Vector3d(0.13, 0, -0.30), world_frame, swing_toe_pos - eps_vec, swing_toe_pos + eps_vec);
          }
          else if(mode_index==1)
          {
          ik.AddPositionConstraint(toe_right_frame, Vector3d(0.13, 0, -0.30), world_frame, stance_toe_pos - eps_vec, stance_toe_pos + eps_vec);
          ik.AddPositionConstraint(toe_left_frame, Vector3d(0.13, 0, -0.30), world_frame, swing_toe_pos - eps_vec, swing_toe_pos + eps_vec);
          }
...
// //添加了关节限位约束的逆运动学数学规划 才是有意义的
  ...
  ik.get_mutable_prog()->AddLinearConstraint((ik.q())(positions_map.at("joint_right_knee")) <= 0.8);
        ...
        if (visualize_init_guess) 
        {
            ...
            string full_name = drake::FindResourceOrThrow("drake/examples/v10/threeD_v2_flaypart_show.urdf");
...
int main(int argc, char* argv[]) 
{
    ...
    std::string full_name = drake::FindResourceOrThrow("drake/examples/v10/threeD_v2_flaypart_show.urdf");//
    drake::FindResourceResult ResourceResult = drake::FindResource("drake/examples/v10/threeD_v2_flaypart_show.urdf");//
    ...
    parser_vis.AddModelFromFile(drake::FindResourceOrThrow("drake/examples/v10/warehouse_show.sdf"));
    ...
    bool yes=plant->HasModelInstanceNamed("threeD_v2_flaypart_show");
    std::cout << "\t model_index "<< "threeD_v2_flaypart_show" <<" has been installed ?: "  <<yes<< std::endl; 
    std::cout <<std::endl;
    //joints 9
    ...
    int M = 10;//对与第二个mode给他20个点,因为他的时长是mode0的两倍
    ...
    out_ramdom_x_file.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/random_x_search112_fixed_ankle.txt", std::ios::out | std::ios::trunc);
    out_ramdom_u_file.open("/home/cby/drake_learning/src/drake_learning_show/src/searchdata/random_u_search112_fixed_ankle.txt", std::ios::out | std::ios::trunc);
    ...
    for (int i = 0; i < 19; i++)//38knots, 37段polynomial,37timesteps
    {
      ...
      //  {
        ...
        int result_only_u = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_u_search112_fixed_ankle.txtxxx", 0);
        //int result_only_u = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/u_planed_outsearch.txtxxx", 0);
        if(result_only_u==0)
        {
          double data_u_ik;
          std::fstream in_u_txt_file_ik;
          in_u_txt_file_ik.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_u_search112_fixed_ankle.txt", std::ios::out | std::ios::in);
          //in_u_txt_file_ik.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/u_planed_outsearch.txt", std::ios::out | std::ios::in);
        ...
        int result_only_x = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_x_search112_fixed_ankle.txtxxx", 0);
        if(result_only_x==0)//在这里我们可以引入缩放的鸵鸟状态先验
        {
          double data_x_ik;
          std::fstream in_x_txt_file_ik;
          in_x_txt_file_ik.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_x_search112_fixed_ankle.txt", std::ios::out | std::ios::in);
   ...
   int result1 = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_x_search112_fixed_anklexxx.txt", 0);//加入先验条件时,需要撤去文件名称的xxx
   int result2 = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_u_search112_fixed_anklexxx.txt", 0);
  if(result1==0&&result2==0)
  {
      std::cout<<"exist prior files xu"<<std::endl;
      std::fstream in_x_txt_file;
      std::fstream in_u_txt_file;
      in_x_txt_file.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_x_search112_fixed_ankle.txt", std::ios::out | std::ios::in);
      in_u_txt_file.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/random_u_search112_fixed_ankle.txt", std::ios::out | std::ios::in);
    ...
    for (int j = 0; j < 2; j++) //3 modes
    {
        ...
        else if(j==1)
        {
            ...
            int result3 = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/lammda_planed_outsereach_select112_fixed.txtxxx", 0);//加入先验条件时,需要撤去文件名称的xxx
            int result4 = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/lammda_c_planed_outsereach_select112_fixed.txtxxx", 0);
            int result5 = access("/home/cby/drake_learning/src/drake_learning_show/src/priordata/gammda_c_planed_outsereach_select112_fixed.txtxxx", 0);
          if(result3==0&&result4==0&&result5==0)
          {
              ...
              in_lammda_data_file.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/lammda_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::in);
              in_lammda_c_data_file.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/lammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::in);
              in_gamma_c_data_file.open("/home/cby/drake_learning/src/drake_learning_show/src/priordata/gammda_c_planed_outsereach_select112_fixed.txt", std::ios::out | std::ios::in);
