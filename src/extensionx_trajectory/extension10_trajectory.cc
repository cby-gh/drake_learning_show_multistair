...
#include "extension10_trajectory.h"
...
namespace cby{
    ...
    void extension6_trajectory<T>::copystateout(const drake::systems::Context<T>& context, drake::systems::BasicVector<T>* output)const
    {
        ...
        if((current_time_copy>=0)&&(current_time_copy<=1.5))
        flag=0;
        else if((current_time_copy>1.5)&&(current_time_copy<=3))        
        flag=1;
        else if((current_time_copy>3)&&(current_time_copy<=4.5))        
        flag=2;
        else if((current_time_copy>4.5)&&(current_time_copy<=6))        
        flag=3;
        else if((current_time_copy>6)&&(current_time_copy<=7.5))        
        flag=4;
        else if((current_time_copy>7.5)&&(current_time_copy<=9))        
        flag=5;
        if((current_time_copy>=0)&&(current_time_copy<=1.5))
        {
        ...
        else if(current_time_copy>1.5)
        {
            T t = current_time_copy-flag*1.5;
            drake::MatrixX<T>  traj_values = trajectory_.value(t);
            //std::cout<<"current_time_copy"<<current_time_copy<<std::endl;
            if(flag%2==0)//偶数相,红色(左足)支撑相->蓝色(右足)支撑相
            {
                cache.SetAtIndex(0,traj_values(0,0)+flag*0.2);//planar_x增加,其余不变
                cache.SetAtIndex(1,traj_values(1,0));
                cache.SetAtIndex(2,traj_values(2,0));
                cache.SetAtIndex(3,traj_values(3,0));
                cache.SetAtIndex(4,traj_values(4,0));
            ...
            else if(flag%2==1)//奇数相,蓝色(右足)支撑相->红色(左足)支撑相,左右腿交换
            {
                cache.SetAtIndex(0,traj_values(0,0)+flag*0.2);//planar_x增加,其余不变
