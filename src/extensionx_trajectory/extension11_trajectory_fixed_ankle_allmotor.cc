...
#include "extension11_trajectory_fixed_ankle_allmotor.h"
...
namespace cby{
    ...
    void extension6_trajectory<T>::copystateout(const drake::systems::Context<T>& context, drake::systems::BasicVector<T>* output)const
    {
        ...
        drake::systems::BasicVector<T>  cache(15);//plant_.num_positions()
...
//1hz
        ...
        if(flag<=10)
        {
            ...
            cache.SetAtIndex(2,traj_values(2,0));
            cache.SetAtIndex(3,traj_values(3,0));
            //cache.SetAtIndex(4,traj_values(4,0)+0.5*flag);4*stride_length
            cache.SetAtIndex(4,traj_values(4,0)+flag*0.25);//2*stride_length 
            cache.SetAtIndex(5,traj_values(5,0));
            cache.SetAtIndex(6,traj_values(6,0)+flag*0.10);
            cache.SetAtIndex(7,traj_values(7,0));
