...
//std::cout<<"  plant_.GetVelocities(*context)00000:"<<plant_.GetVelocities(*context)<<std::endl;
  context->FixInputPort(
      plant_.get_applied_generalized_force_input_port().get_index(),
      J_transpose_lambda);
//std::cout<<"plant_.get_applied_generalized_force_input_port().get_index()"<<plant_.get_applied_generalized_force_input_port().get_index()<<std::endl;
 //   std::cout<<"plant_.get_actuation_input_port().get_index()"<<plant_.get_actuation_input_port().get_index()<<std::endl;
  ...
  const VectorX<T>& v_dot =
      plant_.get_generalized_acceleration_output_port()
          ...
          .CopyToVector();
 //    const VectorX<T>& v_test =
    //  plant_.get_state_output_port()
//          .template Eval<drake::systems::BasicVector<double>>(*context)
//          .CopyToVector();
  //std::cout<<" v_testv_testv_testport55555:"<<v_test.segment(8, 8)<<std::endl;
  VectorX<T> x_dot(plant_.num_positions() + plant_.num_velocities());
  VectorX<T> q_dot(plant_.num_positions());
  //std::cout<<"  plant_.GetVelocities(*context)11111:"<<plant_.GetVelocities(*context)<<std::endl;
    
  plant_.MapVelocityToQDot(*context, plant_.GetVelocities(*context), &q_dot);
    //std::cout<<"q_dotq_dotq_dotq_dot22222:"<<q_dot<<std::endl;
  x_dot << q_dot, v_dot;
  return x_dot;
}
