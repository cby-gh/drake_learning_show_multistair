...
void QuaternionConstraint<T>::EvaluateConstraint(
  ...
  //*y << sqrt(x.squaredNorm() + 1e-3) - sqrt(1 + 1e-3);//squaredNorm 平方L2范数, 
...
//std::cout<<"quat_slack.size() "<<quat_slack.size()<<std::endl; 
  ...
  for (uint i = 0; i < quat_start_indices_.size(); i++) {
    g.segment(quat_start_indices_.at(i), 4) +=
        ...
        //std::cout<<"quat_start_indices_.at(i)"<<quat_start_indices_.at(i)<<std::endl; 
  }
